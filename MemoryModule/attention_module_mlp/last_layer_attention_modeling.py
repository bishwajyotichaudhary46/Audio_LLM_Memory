...
MAL decoder layers (layer_idx % 2 != 0 and layer_idx > 2), additionally ON TOP:
    → mem self-attn (private DynamicCache, never touches EncoderDecoderCache)
    → LMMBlock

Key design points
=================
1. FFN is never removed or replaced — LMMBlock is purely additive.
2. self_attn_mem uses layer_idx=0 in its own private DynamicCache (_mem_attn_cache)
   so it never collides with self.self_attn or encoder_attn slots.
...
FIX 1 — AttributeError: 'DynamicCache' object has no attribute 'is_updated'
    All three attention classes (WhisperAttention, WhisperFlashAttention2,
    WhisperSdpaAttention) now check isinstance(past_key_value, EncoderDecoderCache)
    before accessing .is_updated. Plain DynamicCache (used by _mem_attn_cache)
    takes a direct path with is_updated=False.

FIX 2 — torch.utils.checkpoint.CheckpointError: recomputed tensors have
         different metadata (shape doubled, e.g. [32,12,35,64] → [32,12,70,64])
    Root cause: _mem_attn_cache is mutable state that grows during the forward
    pass. Gradient checkpointing recomputes the forward, but by then the cache
    already holds tokens from the original forward, so it doubles in size.
    Solution:
      a) In WhisperDecoderLayer.forward() the updated cache is captured in a
         local variable and assigned to self._mem_attn_cache AFTER the attention
         call returns (not inside it), keeping the call itself side-effect-free
         with respect to cache growth visible to the checkpoint machinery.
      b) In WhisperDecoder.forward() the make_ckpt_call() closure snapshots
         _mem_attn_cache BEFORE the checkpointed call and restores it at the
         top of ckpt_call() so the recompute always sees the identical cache
         state as the original forward pass.
"""
...
#  Decoder layer  — full Titans MAL integration


class WhisperDecoderLayer(nn.Module):
    """
    Whisper decoder layer with Titans MAL extension.

    ALL layers run:
        [self-attn] → [cross-attn] → [FFN]

    MAL layers (layer_idx % 2 != 0 and layer_idx > 2) additionally run ON TOP:
        → [mem self-attn] → [LMMBlock]

    The mem self-attn and LMMBlock are purely additive — FFN is never
    removed or replaced.

    mem self-attn KV cache
    ----------------------
    self_attn_mem uses layer_idx=0 inside its own private _mem_attn_cache
    (a DynamicCache owned by this layer).  It never touches the shared
    ...
    because gradient checkpointing recomputes the forward pass and the
    cache would double in size on recomputation.  The updated cache is
    therefore captured in a local variable and assigned to
    self._mem_attn_cache only after the attention call returns.
    The WhisperDecoder.forward() closure additionally snapshots the cache
    state before each checkpointed call and restores it at the top of
    ckpt_call() so the recompute always sees the same cache as the
    ...
    def __init__(self, config: WhisperConfig, layer_idx: int = None):
        ...
        if self.use_mem:
            ...
            self.mem_attn_layer_norm = nn.LayerNorm(self.embed_dim)


           
            self.self_attn_mem = WHISPER_ATTENTION_CLASSES[config._attn_implementation](
                embed_dim=self.embed_dim,
                num_heads=config.decoder_attention_heads,
                dropout=config.attention_dropout,
                is_decoder=True,
            ...
            self.final_layer_norm_mem = nn.LayerNorm(self.embed_dim)
    ...
    def forward(
        ...
        if self.use_mem:
            ...
            # Read cache state BEFORE the call (snapshot for grad-ckpt safety)
            cache_in = self._mem_attn_cache

            residual      = hidden_states
            hidden_states = self.mem_attn_layer_norm(hidden_states)

            mem_sa_out, mem_sa_weights, updated_cache = self.self_attn_mem(
                hidden_states=hidden_states,
                past_key_value=cache_in,       
                attention_mask=attention_mask,
                layer_head_mask=layer_head_mask,
                output_attentions=output_attentions,
                cache_position=cache_position,
            ...
            mem_sa_out    = nn.functional.dropout(mem_sa_out, p=self.dropout, training=self.training)

            

            #FFN
            residual_1 = mem_sa_out
            hidden_states = self.final_layer_norm_mem(mem_sa_out)
            hidden_states = self.activation_fn(self.fc11(hidden_states))
            hidden_states = nn.functional.dropout(hidden_states, p=self.activation_dropout, training=self.training)
            hidden_states = self.fc22(hidden_states)
            hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
            hidden_states = residual_1 + hidden_states  + residual                          


            # ff_out        = self.ff_block( )  # FF on top of mem SA output
            # hidden_states = residual + ff_out                          

            if output_attentions:
                self_attn_weights = mem_sa_weights
        ...
        outputs += (new_mem_state,)     # None for non-MAL layers
...
class WhisperDecoder(WhisperPreTrainedModel):
    ...
    _mem_attn_cache on each MAL WhisperDecoderLayer:
        Private DynamicCache for self_attn_mem.
    ...
    preventing the shape-doubling CheckpointError.
    """
    ...
    def reset_mem_states(self) -> None:
        ...
        Reset ALL transient decode state between utterances:
          ...
          - Private _mem_attn_cache for every MAL layer
