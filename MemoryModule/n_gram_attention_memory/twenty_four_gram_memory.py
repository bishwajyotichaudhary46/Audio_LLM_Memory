# coding=utf-8
"""
Whisper + Stateful Sliding-Window Queue Adapter
================================================

This file replaces the previous Titans/LMM `mem_block` path with a small
stateful sliding-window key-queue adapter inside selected Whisper decoder
layers.

Main behavior
-------------
1. Standard Whisper encoder/decoder is preserved.
2. Decoder layers after layer index 2 get a trainable queue adapter.
3. Queue size is 8: previous 7 decoder keys + current decoder key.
4. Queue runtime state is reset at the start of every new utterance.
5. Queue runtime state is carried during token-by-token generation.
6. No external MemoryModule/Queue classes are required.

Important distinction
---------------------
- Queue weights are trainable and should NOT be reinitialized each utterance.
- Queue states are runtime FIFO buffers and ARE reset each utterance.
"""
...
class SlidingWindowKeyQueueLayer(nn.Module):
    """
    Stateful sliding-window key-queue adapter for Whisper decoder states.

    Full-sequence training:
        hidden_states: [B, T, D]
        state: None
        The queue windows are built causally from the full sequence.

    Token-by-token generation:
        hidden_states: [B, 1, D]
        state: previous key queue [B, <= queue_size - 1, key_dim]
        The adapter remembers the last 7 keys when queue_size=8.

    Returns:
        update: [B, T, D]
        new_state: [B, <= queue_size - 1, key_dim]
    """
    ...
    def __init__(
        ...
        self.flat_dim = queue_size * key_dim

        self.norm = nn.LayerNorm(embed_dim)
        self.key_proj = nn.Linear(embed_dim, key_dim, bias=False)

        # Learnable representation for "empty history" positions.
        self.pad_key = nn.Parameter(torch.zeros(1, key_dim))
        nn.init.normal_(self.pad_key, mean=0.0, std=0.02)

        # Queue context -> hidden update.
        self.queries = nn.Linear(self.flat_dim, inner_dim, bias=False)
        self.values = nn.Linear(inner_dim, embed_dim, bias=False)

        self.scale = 1.0 / math.sqrt(self.flat_dim)

    def reset_parameters_for_adapter(self) -> None:
        """
        Called after Hugging Face post_init(), because post_init() reinitializes
        Linear layers and would otherwise overwrite our stable adapter init.
        """
        nn.init.normal_(self.pad_key, mean=0.0, std=0.02)
        nn.init.zeros_(self.values.weight)  # start as no-op adapter
    ...
    def _build_windows(
        ...
        """
        Build causal queue windows.

        keys:
            [B, T, key_dim]
        state:
            [B, <= queue_size - 1, key_dim] or None
        """
        B, T, D = keys.shape
        Q = self.queue_size

        if state is not None:
            # Beam size / batch size may change during generation.
            if state.size(0) != B:
                state = None
            else:
                state = state.to(device=keys.device, dtype=keys.dtype)

        if state is None:
            context = keys
            prev_len = 0
        else:
            context = torch.cat([state, keys], dim=1)
            prev_len = state.size(1)

        # Left pad with Q-1 learnable pad keys, then unfold sliding windows.
        pad = self.pad_key.unsqueeze(0).expand(B, Q - 1, D)
        padded = torch.cat([pad, context], dim=1)  # [B, Q-1+L, D]

        all_windows = padded.unfold(1, Q, 1)       # [B, L, D, Q]
        all_windows = all_windows.permute(0, 1, 3, 2).contiguous()  # [B, L, Q, D]

        windows = all_windows[:, prev_len:prev_len + T, :, :]

        keep = min(Q - 1, context.size(1))
        new_state = context[:, -keep:, :].contiguous()

        return windows, new_state
    ...
    def forward(
        ...
        keys = self.key_proj(normed)  # [B, T, key_dim]

        windows, new_state = self._build_windows(keys, state=state)

        B, T, Q, KD = windows.shape
        flat = windows.reshape(B, T, Q * KD)

        scores = self.queries(flat) * self.scale
        attn = F.softmax(scores, dim=-1)
        update = self.values(attn)
...
# Decoder layer — Whisper queue adapter integration
class WhisperDecoderLayer(nn.Module):
    """
    Whisper decoder layer with a trainable sliding-window queue adapter.

    Layer structure for use_mem=True:
        [self-attn] -> [cross-attn] -> [standard Whisper FFN] -> [queue adapter]

    The queue adapter is gated using router_proj and uses a causal FIFO queue
    of projected decoder keys. With queue_size=8, each token sees the previous
    7 decoder-token keys plus its current key.
    """
    ...
    def __init__(self, config: WhisperConfig, layer_idx: int = None):
        ...
        # Standard Whisper FFN.
        self.fc1              = nn.Linear(self.embed_dim, config.decoder_ffn_dim)
        self.fc2              = nn.Linear(config.decoder_ffn_dim, self.embed_dim)
        self.final_layer_norm = nn.LayerNorm(self.embed_dim)

        # Queue adapter is inserted in place of the old mem_block path.
        # Same selection as your earlier logic: layers after index 2.
        self.use_mem = (
        ...
        if self.use_mem:
            ...
            self.queue_layer = SlidingWindowKeyQueueLayer(
                embed_dim=config.d_model,
                key_dim=getattr(config, "queue_key_dim", 128),
                queue_size=getattr(config, "queue_size", 24),
                inner_dim=getattr(config, "queue_inner_dim", 2048),
            )
    ...
    def forward(
        ...
        queue_state:                Optional[torch.Tensor]        = None,
        ...
        Tuple whose elements are:
            (hidden_states,
             ...
             new_queue_state)         # always last; None for non-queue layers
        """
        ...
        # gate = None
        # if self.use_mem:
        #     gate_logits = self.router_proj(hidden_states)   # [B, T, 1]
        ...
        new_queue_state: Optional[torch.Tensor] = None

        if self.use_mem:
            residual      = hidden_states
            hidden_states = self.mem_attn_layer_norm(hidden_states)
            queue_update, new_queue_state = self.queue_layer(
                hidden_states,
                state=queue_state,
            )
            queue_update = F.dropout(queue_update, p=self.dropout, training=self.training)
            
            hidden_states = residual + queue_update

            #FFN
            residual_1 = hidden_states
            hidden_states = self.final_layer_norm_mem(hidden_states)
            ...
            # final 
            hidden_states = residual + gate*(hidden_states + residual_1)
        ...
        # Pack outputs — new_queue_state is ALWAYS the last element
        ...
        outputs += (new_queue_state,)  # always last; None for non-queue layers
...
# Decoder — Queue state management lives here
class WhisperDecoder(WhisperPreTrainedModel):
    """
    Whisper decoder with per-layer runtime sliding-window queue states.

    queue_states: dict[layer_idx -> torch.Tensor]
        Stores the last queue_size - 1 projected decoder keys for generation.
        Must be reset between utterances via reset_queue_states().
    """
    ...
    def __init__(self, config: WhisperConfig):
        ...
        # Runtime queue state per queue layer. This is not a trainable parameter.
        self.queue_states: dict[int, torch.Tensor] = {}
        # lang embed
        self.lang_embed = None

        self.post_init()
        self._init_queue_adapters()

    def _init_queue_adapters(self) -> None:
        """Stable init for newly added queue adapters after HF post_init()."""
        for layer in self.layers:
            if getattr(layer, "use_mem", False):
                # nn.init.zeros_(layer.router_proj.weight)
                # nn.init.constant_(layer.router_proj.bias, -4.0)
                layer.queue_layer.reset_parameters_for_adapter()

    def reset_queue_states(self) -> None:
        """
        Clear runtime queue states at the START of every new utterance.
        This does not reinitialize trainable queue adapter weights.
        """
        self.queue_states = {}

    def reorder_queue_states(self, beam_idx: torch.LongTensor) -> None:
        """Keep queue states aligned during beam search reordering."""
        for idx, state in list(self.queue_states.items()):
            self.queue_states[idx] = state.index_select(0, beam_idx.to(state.device))
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            # Retrieve stored queue state for this layer. During full-sequence
            # training/evaluation without cache, we rebuild windows from the
            # current sequence and do not persist queue states.
            current_queue_state = self.queue_states.get(idx, None) if use_cache else None
            ...
            # new_queue_state is always the last element. Store only during
            # cached generation, not full-sequence training.
            new_queue_state: Optional[torch.Tensor] = layer_outputs[-1]
            if use_cache and new_queue_state is not None:
                self.queue_states[idx] = new_queue_state.detach()
...
class WhisperModel(WhisperPreTrainedModel):
    def __init__(self, config: WhisperConfig):
        ...
        self.post_init()
        self.decoder._init_queue_adapters()
...
class WhisperForConditionalGeneration(WhisperGenerationMixin, WhisperPreTrainedModel):
    ...
    def __init__(self, config: WhisperConfig):
        ...
        self.post_init()
        self.model.decoder._init_queue_adapters()
    ...
    def forward(
        ...
        # Reset runtime queue at the start of every new utterance.
        # past_key_values is None for training and the first generate() step.
        if past_key_values is None:
            self.model.decoder.reset_queue_states()
    ...
    def _reorder_cache(self, past_key_values, beam_idx):
        """
        Reorder both Hugging Face KV cache and our custom queue states during beam search.
        """
        if hasattr(self.model.decoder, "reorder_queue_states"):
            self.model.decoder.reorder_queue_states(beam_idx)
