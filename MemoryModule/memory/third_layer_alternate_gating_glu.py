...
class WhisperDecoderLayer(nn.Module):
    ...
    def __init__(self, config: WhisperConfig, layer_idx: int = None):
        ...
        self.use_mem = (layer_idx >= 2 and layer_idx % 2 != 0)
        # self.use_mem = (layer_idx is not None) and (layer_idx % 2 == 0)

        if self.use_mem:
            # self.alpha = nn.Parameter(torch.tensor(1e-3))
            self.router_gate = nn.Linear(2*config.d_model, config.d_model, bias=True)
            self.router_value = nn.Linear(2*config.d_model, config.d_model, bias=True)
    ...
    def forward(
        ...
        if self.use_mem:
            ...
            router_gate = self.router_gate(gate_states)
            router_value = self.router_value(gate_states)
