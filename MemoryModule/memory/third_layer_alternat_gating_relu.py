...
class WhisperDecoderLayer(nn.Module):
    ...
    def __init__(self, config: WhisperConfig, layer_idx: int = None):
        ...
        if self.use_mem:
            ...
            self.router_proj_fc1 = nn.Linear(config.d_model, config.d_model//2, bias=True)
            self.router_proj_fc2 = nn.Linear(config.d_model//2, 1, bias=True)
    ...
    def forward(
        ...
        if self.use_mem:
            ...
            gate_logits = self.router_proj_fc2(F.relu(self.router_proj_fc1(gate_states)))   # (bs, seq, 1)
            gate = torch.sigmoid(gate_logits)
