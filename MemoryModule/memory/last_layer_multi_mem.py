...
2.  LMMBlock is APPENDED after the decoder FFN in the final layer
    ----------------------------------------------------------------
    Your requested design keeps the normal Whisper decoder FFN.
    Only the final decoder layer receives three stacked LMMBlocks after FFN:
        [self-attn] → [cross-attn] → [Whisper FFN]
        → [LMMBlock 1] → [LMMBlock 2] → [LMMBlock 3]

3.  Three external gates added for stacked memory blocks
    -----------------------------------------------------
    Each stacked LMMBlock has its own router/gate:
        hidden_states = hidden_states + sigmoid(router([mem_out, hidden_states])) * mem_out
    Gate weights are re-initialized after Hugging Face post_init() so the
    model starts close to normal Whisper.
...
class WhisperDecoderLayer(nn.Module):
    """
    Whisper decoder layer with stacked memory only in the final decoder layer.

    For normal decoder layers:
        Self-Attention
        → Cross-Attention
        → Whisper FFN

    For final decoder layer:
        Self-Attention
        → Cross-Attention
        → Whisper FFN
        → Memory Block 1 + Gate
    ...
    Important:
        This version does NOT replace the FFN.
        FFN always runs first.
        Memory is appended after FFN only in the final decoder layer.
    """
    ...
    def __init__(self, config: WhisperConfig, layer_idx: int = None):
        ...
        # This is kept for ALL layers, including the final memory layer.
        self.fc1 = nn.Linear(self.embed_dim, config.decoder_ffn_dim)
        self.fc2 = nn.Linear(config.decoder_ffn_dim, self.embed_dim)
        self.final_layer_norm = nn.LayerNorm(self.embed_dim)

        # Add stacked memory only in the final decoder layer.
        self.use_mem = (
        ...
        if self.use_mem:
            self.num_stacked_mem_blocks = 3

            self.mem_blocks = nn.ModuleList()
            # self.router_projs = nn.ModuleList()
            self.router_proj = nn.Linear(2 * config.d_model, 1, bias=True)

            for _ in range(self.num_stacked_mem_blocks):
                titans_cfg = TitansConfig(
                    ...
                    # Each LMMBlock is one stacked memory block.
                    # Keep this as 1 to avoid 3 internal memory layers
                    # inside each of the 3 stacked blocks.
                    num_memory_layers=1,
                )

                self.mem_blocks.append(LMMBlock(titans_cfg))
    ...
    def forward(
        ...
        mem_state: Optional[Union[MemoryState, List[MemoryState]]] = None,
        lang_embed: Optional[torch.Tensor] = None,
    ) -> Tuple:
        """
        Output tuple:
            hidden_states
            self_attn_weights, cross_attn_weights    optional
            present_key_value                         optional
            new_mem_state                             always last

        For final layer:
            new_mem_state is a list:
                [
                    state_for_memory_block_1,
                    state_for_memory_block_2,
                    state_for_memory_block_3,
                ]

        For non-memory layers:
            new_mem_state = None
        """
        ...
        if encoder_hidden_states is not None:
            ...
            normed_hidden_states = self.encoder_attn_layer_norm(hidden_states)

            cross_attn_out, cross_attn_weights, cross_attn_pkv = self.encoder_attn(
                hidden_states=normed_hidden_states,
            ...
            cross_attn_out = nn.functional.dropout(
                cross_attn_out,
                p=self.dropout,
                training=self.training,
            )
        ...
        new_mem_state: Optional[Union[MemoryState, List[MemoryState]]] = None

        if self.use_mem:
            # mem_state may be None on the first step.
            # After that, it becomes a list of 3 states.
            if mem_state is None:
                mem_state = [None] * self.num_stacked_mem_blocks

            elif not isinstance(mem_state, (list, tuple)):
                # Safety fallback for older single-state format.
                mem_state = [mem_state] + [None] * (self.num_stacked_mem_blocks - 1)

            else:
                # Make sure we always have exactly 3 states.
                mem_state = list(mem_state)
                if len(mem_state) < self.num_stacked_mem_blocks:
                    mem_state = mem_state + [None] * (self.num_stacked_mem_blocks - len(mem_state))
                elif len(mem_state) > self.num_stacked_mem_blocks:
                    mem_state = mem_state[: self.num_stacked_mem_blocks]

            new_mem_states = []
            mem_in = hidden_states
            for i, mem_block in enumerate(self.mem_blocks):
                mem_out, state_i = mem_block(
                    mem_in,
                    encoder_hidden_states=encoder_hidden_states,
                    state=mem_state[i],
                )
                ...
                mem_in =  mem_out
                new_mem_states.append(state_i)

            gate_input = torch.cat(
                    [mem_out, hidden_states],
                    dim=-1,
                )

            gate_logits = self.router_proj(gate_input)
            gate = torch.sigmoid(gate_logits)
            hidden_states = hidden_states + gate * mem_out
            new_mem_state = new_mem_states
        ...
        # Always keep memory state as the last output.
        outputs += (new_mem_state,)
...
class WhisperDecoder(WhisperPreTrainedModel):
    ...
    def __init__(self, config: WhisperConfig):
        ...
        self.mem_states: dict[int, Union[MemoryState, List[MemoryState]]] = {}
        # lang embed
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            new_mem_state: Optional[Union[MemoryState, List[MemoryState]]] = layer_outputs[-1]
