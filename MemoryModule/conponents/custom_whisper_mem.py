...
class WhisperDecoderLayer(nn.Module):
    def __init__(self, config: WhisperConfig, layer_idx: int = None,   num_memories: int = 8,
        bias_term_adjust = None,
        LinearAttentionMem = None,
        ...
        if is_memory_layer:
            # self.attn_route = WHISPER_ATTENTION_CLASSES[config._attn_implementation](
            #     embed_dim=self.embed_dim,
            #     num_heads=config.decoder_attention_heads,
            #     dropout=config.attention_dropout,
            #     is_decoder=True,
    ...
    def forward(
        ...
        if self.is_memory_layer:
            ...
            #hidden_states = self.self_attn_layer_norm(residual)
            
            # Routing gate
            #hidden_states = self.attn_route(hidden_states)  # [B, T, 1]
            logits = self.router_proj(hidden_states)
            gate = torch.sigmoid(logits)
            # print("logits: ", logits)
            # print("attn gate:", gate)
            #gate = torch.clip(gate, min=0, max=0)
            # Update via MOM
            mem_out = self.context_mom(residual)
            mem_out = self.memory_norm(mem_out)
       
            hidden_states = hidden_states + nn.functional.dropout(gate * mem_out, p=self.dropout, training=self.training)
...
class WhisperDecoder(WhisperPreTrainedModel):
    ...
    def __init__(self, config: WhisperConfig,
        ...
        self.layers = nn.ModuleList(
            [WhisperDecoderLayer(config, layer_idx,num_memories=num_memories, 
                          bias_term_adjust= bias_term_adjust, 
                          LinearAttentionMem=LinearAttentionMem,
                          is_memory_layer=(layer_idx == 5) or (layer_idx==7) ) for layer_idx in range(config.decoder_layers)]
        )
