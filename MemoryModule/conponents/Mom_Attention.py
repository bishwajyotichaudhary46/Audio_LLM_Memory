...
def _upad_input(
    ...
    if query_length == kv_seq_len:
        ...
        # Slice mask for query side
        attention_mask_q = attention_mask[:, -query_length:]

        # Get unpad metadata for Q
        indices_q, cu_seqlens_q, max_seqlen_in_batch_q = get_unpad_data(attention_mask_q)
        ...
        query_layer = index_first_axis(
            query_layer.reshape(batch_size * query_length, dim),
            indices_q 
            )

        # # The -q_len: slice assumes left padding.
        # attention_mask = attention_mask[:, -query_length:]
        # query_layer, indices_q, cu_seqlens_q, max_seqlen_in_batch_q = unpad_input(q=query_layer, attention_mask=attention_mask)
...
class MomAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        # print("hidden states after cu2pad", hidden_states.shape)
        ...
        # 🔍 topk gating
        # print("hidden state before gating", hidden_states)
        ...
        print("kv_states", kv_states.shape)
        if self.single_kv_proj:
            k = self.shared_k(kv_states)
            v = self.shared_v(kv_states)
            beta = self.shared_b(kv_states).sigmoid()
            g = -self.A_log.float().exp() * F.softplus(self.shared_a(kv_states).float() + self.dt_bias)
        else:
            k = torch.stack([k_expert(kv_states[i]) for i, k_expert in enumerate(self.k_proj)], dim=0)
            v = torch.stack([v_expert(kv_states[i]) for i, v_expert in enumerate(self.v_proj)], dim=0)
            beta = torch.stack([b_expert(kv_states[i]).sigmoid() for i, b_expert in enumerate(self.b_proj)], dim=0)
            g = torch.stack([-self.A_log.float().exp() * F.softplus(a_expert(kv_states[i]).float() + self.dt_bias)
        ...
        beta = self.shared_b(kv_states).sigmoid()
        g = -self.A_log.float().exp() * F.softplus(self.shared_a(kv_states).float() + self.dt_bias)
        # print("q shape before rearrange", q.shape)
        # print("k shape before rearrange", k.shape)
        # print("v shape before rearrange", v.shape)
        # print("g shape before rearrange", g.shape)
        # print("beta shape before rearrange", beta.shape)
        # print("mask_2 shape before rearrange", mask_2.shape)
        q = (rearrange(q, 'e b l ... ->  (e b) l ...') )
        # print("q shape after rearrange", q.shape)
        # print("k shape after rearrange", k.shape)
        # print("v shape after rearrange", v.shape)
        # print("g shape after rearrange", g.shape)
        # print("beta shape after rearrange", beta.shape)
        print("mask_2 shape after rearrange", mask_2.shape)
        ...
        if mode == 'chunk':
            ...
            # print("o after chunk gated delta rulr", o)
