# coding=utf-8
"""
Whisper + Titans + TAS Integration — corrected MAL (Memory as Layer) variant.
...
from MemoryModule.conponents.Context_Mem_TAS_Block import ContextMemTASBlock
...
class WhisperDecoderLayer(nn.Module):
    ...
    def __init__(self, config: WhisperConfig, layer_idx: int = None):
        ...
        if self.use_mem:
            ...
            self.mem_block = ContextMemTASBlock(titans_cfg)
            # NOTE: no mem_gate here — LMMBlock already contains a per-dim
    ...
    def forward(
        ...
        if self.use_mem:
            ...
            mem_out, new_mem_state = self.mem_block(
                hidden_states,
                encoder_hidden_states=encoder_hidden_states,   # ← FIX: was positional
                state=mem_state,
            )
