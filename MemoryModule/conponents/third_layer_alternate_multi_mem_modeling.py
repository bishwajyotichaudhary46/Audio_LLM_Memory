...
class WhisperDecoderLayer(nn.Module):
    """
    Whisper decoder layer with multiple memory blocks per layer.

    For use_mem=True:
        [self-attn] -> [cross-attn] -> [standard FFN] -> [stacked mem blocks]

    Each mem block has:
        - its own LMMBlock
        - its own gate projection
        - its own MemoryState
    ...
    def __init__(self, config: WhisperConfig, layer_idx: int = None):
        ...
        num_memory_layers = getattr(config, "num_memory_layers", 1)
        self.use_mem = (layer_idx is not None) and (layer_idx >= config.decoder_layers - num_memory_layers)

        if self.use_mem:
            num_mem_blocks = getattr(config, "num_mem_blocks_per_layer", 2)
            ...
            self.mem_blocks = nn.ModuleList([
                LMMBlock(titans_cfg) for _ in range(num_mem_blocks)
            ])
    ...
    def forward(
        ...
        mem_state: Optional[list[MemoryState]] = None,
        lang_embed: Optional[torch.Tensor] = None,
    ) -> Tuple:
        """
        mem_state:
            None or list[MemoryState] with length == num_mem_blocks
        """
        ...
        if self.use_mem:
            if mem_state is None:
                mem_state = [None] * self.num_mem_blocks

    
            new_states = []
            mem_input = hidden_states

            for i, mem_block in enumerate(self.mem_blocks):
                block_state = mem_state[i] if mem_state is not None else None

                mem_out, new_state = mem_block(
                    mem_input,
                    encoder_hidden_states=encoder_hidden_states,
                    state=block_state,
                )
                ...
                mem_input = mem_input + mem_out
                new_states.append(new_state)

            hidden_states = hidden_states + gate * mem_input
            new_mem_states = new_states
        ...
        outputs += (new_mem_states,)   # always last
        return outputs
...
class WhisperDecoder(WhisperPreTrainedModel):
    """
    Whisper decoder with persistent multi-memory-block states.

    mem_states: dict[layer_idx -> list[MemoryState]]

    For each decoder layer that uses memory:
        self.mem_states[idx] = [state_block_0, state_block_1, ...]
    """
    ...
    def __init__(self, config: WhisperConfig):
        ...
        # IMPORTANT: list of states per layer
        self.mem_states: dict[int, list[MemoryState]] = {}
    ...
    def forward(
        ...
        # keep language embedding if available
        if input_shape[1] > 1:
            self.lang_embed = inputs_embeds[:, 1, :]   # (B, D)
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            current_mem_states = self.mem_states.get(idx, None)

            if self.gradient_checkpointing and self.training:
                # If this branch causes issues with non-tensor state objects,
                # disable gradient checkpointing for memory-enabled training.
                ...
                layer_outputs = decoder_layer(
                    ...
                    use_cache=use_cache,
                    cache_position=cache_position,
                    mem_state=current_mem_states,
                    lang_embed=self.lang_embed,
                )

            hidden_states = layer_outputs[0]

            # last element is always list[MemoryState] or None
            new_mem_states = layer_outputs[-1]
            if new_mem_states is not None:
                self.mem_states[idx] = new_mem_states
