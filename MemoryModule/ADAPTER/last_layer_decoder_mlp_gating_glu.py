...
class WhisperDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        residual = hidden_states
        hidden_states = self.final_layer_norm(hidden_states)
        hidden_states_adapter = hidden_states
        hidden_states = self.activation_fn(self.fc1(hidden_states))
        hidden_states = nn.functional.dropout(hidden_states, p=self.activation_dropout, training=self.training)
        hidden_states = self.fc2(hidden_states)
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        ...
        if self.use_adapter:
            ...
            router_score = self.router_proj(torch.sigmoid(router_gate) * router_value)
            router_score = torch.sigmoid(router_score)

            hidden_states_adapter = self.activation_fn(self.fc11(hidden_states_adapter))
            hidden_states_adapter = nn.functional.dropout(
                hidden_states_adapter, p=self.activation_dropout, training=self.training
            )
            hidden_states_adapter = self.fc22(hidden_states_adapter)
            hidden_states_adapter = nn.functional.dropout(
                hidden_states_adapter, p=self.dropout, training=self.training
            )

            hidden_states = hidden_states + router_score * hidden_states_adapter
